...
SELECT
    c.FirstName AS [Customer FN], c.LastName AS [Customer LN], e.FirstName AS [Employee FN], e.LastName AS [Employee LN], i.total
FROM
    Invoice i
    INNER JOIN Customer c
    ON i.CustomerId = c.CustomerId INNER JOIN
    Employee e
...
SELECT
    round
(avg
(total),2) AS [Avg Transaction Amount]
FROM
    Invoice
WHERE
InvoiceDate >= '2011-01-01' AND InvoiceDate <='2012-12-31'
...
SELECT
    count
(total) AS [Num of Transactions Above Avg]
FROM
    Invoice
WHERE
total >
...
SELECT
    round(avg(total),2) AS [Avg Transaction Amount], strftime('%Y',InvoiceDate) AS [Year]
FROM
    Invoice
GROUP BY strftime
('%Y',InvoiceDate)
...
WHERE
InvoiceDate >= '2011-01-01'
    AND InvoiceDate <='2012-12-31' AND
    i.total > 11.66
GROUP BY
e.FirstName,
e.LastName
...
--3. Which employee made the highest commission? -- Jane Peacock $106.21
